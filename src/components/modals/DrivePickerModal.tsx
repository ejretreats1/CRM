  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[80vh] flex flex-col">
        ...
        {/* Breadcrumbs */}
        <nav className="flex items-center gap-1 px-5 py-2.5 border-b border-slate-100 flex-shrink-0 bg-slate-50 flex-wrap">
          {crumbs.map((crumb, i) => (
            <span key={i} className="flex items-center gap-1">
              {i > 0 && <ChevronRight size={13} className="text-slate-300" />}
              <button
                onClick={() => setCrumbs(prev => prev.slice(0, i + 1))}
                className={`text-xs flex items-center gap-1 px-2 py-0.5 rounded hover:bg-slate-200 transition-colors ${
                  i === crumbs.length - 1
