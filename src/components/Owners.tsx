...
export default function Owners({ owners, onViewOwner, onOpenOwnerModal, onDeleteOwner, onArchiveOwner }: OwnersProps) {
  ...
  function OwnerCard({ owner, isArchived }: { owner: Owner; isArchived: boolean }) {
    ...
    return (
      <div
        onClick={() => !isArchived && onViewOwner(owner.id)}
        className={`bg-[#1a2335] rounded-xl border border-[#1e2d45] p-5 transition-all group ${
          isArchived
            ? 'opacity-60 cursor-default'
            : 'cursor-pointer hover:shadow-md hover:border-[#1e3a5a]'
        }`}
      >
        <div className="flex items-start gap-4">
          {/* Avatar */}
          <div className={`w-11 h-11 rounded-full flex items-center justify-center flex-shrink-0 ${isArchived ? 'bg-[#2a3a50]' : 'bg-[#4a90d9]'}`}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2 min-w-0">
                ...
                {isArchived && (
                  <span className="text-xs bg-[#1e2d45] text-[#3a5070] px-2 py-0.5 rounded-full flex-shrink-0">
                    Archived
                  </span>
                )}
              </div>
              <div className="flex items-center gap-1 flex-shrink-0">
                {isArchived ? (
                  <>
                    <button
                      onClick={e => { e.stopPropagation(); onArchiveOwner(owner.id, false); }}
                      className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1 text-xs text-[#4ab57a] border border-[#0a2518] hover:bg-[#0a2518] px-2 py-1 rounded-lg"
                    ...
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 transition-opacity text-[#3a5070] hover:text-[#e05c5c] p-1.5"
                  ...
                  <>
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 transition-opacity text-[#3a5070] hover:text-[#d0954a] p-1.5"
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              {owner.properties.map(p => (
                <span
                  key={p.id}
                  className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_COLORS[p.status] ?? STATUS_COLORS.inactive}`}
                >
          ...
          <div className="text-right flex-shrink-0 hidden sm:block">
            <div className={`flex items-center gap-1 font-bold ${isArchived ? 'text-[#3a5070]' : 'text-[#4a90d9]'}`}>
              <TrendingUp size={14} />
              ${rev.toLocaleString()}
            </div>
    ...
    <div className="p-4 sm:p-6 max-w-5xl mx-auto space-y-5">
      ...
      <div className="space-y-3">
        ...
        {filteredActive.map(owner => (
          <OwnerCard key={owner.id} owner={owner} isArchived={false} />
        ))}
      </div>
          ...
          {showArchived && (
            <div className="space-y-3">
              {filteredArchived.length === 0 ? (
                <p className="text-sm text-[#3a5070] text-center py-6">No archived clients.</p>
              ) : (
                filteredArchived.map(owner => (
                  <OwnerCard key={owner.id} owner={owner} isArchived={true} />
                ))
              )}
            </div>
          )}
