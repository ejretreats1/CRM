    ...
    <div className="p-4 sm:p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div>
        ...
        <div className="flex items-start gap-4">
          <div className="w-14 h-14 rounded-full bg-[#4a90d9] flex items-center justify-center flex-shrink-0">
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-4 mt-1.5">
              {owner.email && <a href={`mailto:${owner.email}`} className="flex items-center gap-1.5 text-sm text-[#4a90d9] hover:underline"><Mail size={14} /> {owner.email}</a>}
              {owner.phone && <a href={`tel:${owner.phone}`}   className="flex items-center gap-1.5 text-sm text-[#4a90d9] hover:underline"><Phone size={14} /> {owner.phone}</a>}
            </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Monthly Revenue', value: `$${totalRevenue.toLocaleString()}`, icon: TrendingUp, color: 'text-[#4a90d9]' },
          { label: 'Properties',      value: owner.properties.length,              icon: Home,       color: 'text-indigo-500' },
          { label: 'Avg Occupancy',   value: avgOccupancy ? `${avgOccupancy}%` : '—', icon: Wifi, color: 'text-[#d0954a]' },
        ].map(s => (
      ...
      <div className="bg-[#1a2335] rounded-xl border border-[#1e2d45]">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#1e2d45] flex-wrap gap-2">
          <h2 className="font-semibold text-white">Properties</h2>
          <div className="flex items-center gap-2 flex-wrap">
            {uplistingApiKey && (
              <button
                onClick={() => openImport('uplisting')}
                className="flex items-center gap-1.5 text-xs text-[#d07af5] hover:text-[#d07af5] border border-[#2a1a5a] hover:border-indigo-400 px-3 py-1.5 rounded-lg transition-colors font-medium"
              >
            ...
            {hostawayAccountId && hostawaySecret && (
              <button
                ...
                className="flex items-center gap-1.5 text-xs text-violet-600 hover:text-[#d07af5] border border-violet-200 hover:border-violet-400 px-3 py-1.5 rounded-lg transition-colors font-medium"
              >
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs text-[#4a90d9] hover:text-[#4a90d9] border border-[#1e3a5a] hover:border-[#4a90d9] px-3 py-1.5 rounded-lg transition-colors font-medium"
            >
        ...
        <div className="divide-y divide-[#1e2d45]">
          ...
          {owner.properties.map(property => {
            ...
            return (
              <div
                key={property.id}
                className={`px-5 py-4 ${onNavigateToProperty ? 'cursor-pointer hover:bg-[#1e2d45] transition-colors' : ''}`}
                ...
                <div className="flex items-start justify-between gap-3">
                    ...
                    {property.platforms.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mt-2">
                        {property.platforms.map(p => (
                          <span key={p} className="text-xs bg-[#162035] text-[#6ab0f5] px-2 py-0.5 rounded-full">{p}</span>
                        ))}
                      </div>
                    )}
                  </div>
                  <div className="text-right flex-shrink-0">
                    {(() => {
                      ...
                      return live != null ? (
                        <>
                          <div className="font-bold text-[#4a90d9]">${live.toLocaleString()}</div>
                          <div className="text-xs text-[#6ab0f5]">30d · live</div>
                        </>
                      ) : (
                        <>
                          <div className="font-bold text-[#4a90d9]">${property.monthlyRevenue.toLocaleString()}</div>
      ...
      <div className="bg-[#1a2335] rounded-xl border border-[#1e2d45]">
          ...
          <div className="px-5 py-4 border-b border-[#1e2d45]">
            ...
            <div
              ...
              className={`border-2 border-dashed rounded-xl p-8 text-center transition-colors ${
                uploading ? 'border-[#1e2d45] bg-[#1e2d45] cursor-default'
            ...
            {uploadError && <p className="text-xs text-[#e05c5c] mt-2 text-center">{uploadError}</p>}
          </div>
        )}
        ...
        <div className="divide-y divide-[#1e2d45]">
          ...
          {revenueReports.map(r => (
            <button key={r.id} onClick={() => setSelectedReport(r)} className="flex items-center gap-3 px-5 py-3.5 w-full text-left hover:bg-[#1e2d45] transition-colors">
              <div className="w-9 h-9 rounded-lg bg-[#162035] flex items-center justify-center flex-shrink-0"><FileBarChart2 size={16} className="text-[#6ab0f5]" /></div>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                {r.airdnaProjectedRevenue != null && <span className="text-sm font-bold text-[#4a90d9]">${Math.round(r.airdnaProjectedRevenue).toLocaleString()}/yr</span>}
          ...
          {ownerDocs.map(doc => (
            <div key={doc.id} className="flex items-center gap-3 px-5 py-3.5">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <a href={doc.fileUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-[#4a90d9] hover:text-[#4a90d9] border border-[#1e3a5a] hover:border-[#4a90d9] px-2.5 py-1.5 rounded-lg transition-colors"><Download size={11} /> View</a>
                <button onClick={() => handleDelete(doc)} className="flex items-center gap-1 text-xs text-[#3a5070] hover:text-[#e05c5c] border border-[#1e2d45] hover:border-[#5a1a1a] px-2.5 py-1.5 rounded-lg transition-colors"><Trash2 size={11} /> Delete</button>
              </div>
            </div>
          ))}
          {driveLinks.map(link => (
            <div key={link.id} className="flex items-center gap-3 px-5 py-3.5">
              <div className="w-9 h-9 rounded-lg bg-[#162035] flex items-center justify-center flex-shrink-0 text-base">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <a href={link.webViewLink} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-[#4a90d9] hover:text-[#4a90d9] border border-[#1e3a5a] hover:border-[#4a90d9] px-2.5 py-1.5 rounded-lg transition-colors"><ExternalLink size={11} /> Open</a>
                <button onClick={() => handleDeleteDriveLink(link)} className="flex items-center gap-1 text-xs text-[#3a5070] hover:text-[#e05c5c] border border-[#1e2d45] hover:border-[#5a1a1a] px-2.5 py-1.5 rounded-lg transition-colors"><Trash2 size={11} /> Remove</button>
              </div>
            </div>
          ))}
          {sigRequests.map(req => {
              ...
              <div key={req.id} className="px-5 py-4">
                <div className="flex items-start gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    {req.signedAt && <p className="text-xs text-[#5ce0a0] mt-0.5">Signed {new Date(req.signedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>}
                  </div>
      ...
      <div className="bg-[#1a2335] rounded-xl border border-[#1e2d45]">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#1e2d45]">
          ...
          <button onClick={() => { setVendorForm({ name: '', role: 'Cleaner', phone: '', email: '', notes: '' }); setEditingVendorId(null); }} className="flex items-center gap-1.5 text-xs text-[#4a90d9] hover:text-[#4a90d9] border border-[#1e3a5a] hover:border-[#4a90d9] px-3 py-1.5 rounded-lg transition-colors font-medium"><Plus size={13} /> Add Vendor</button>
        </div>
        {vendorForm !== null && (
          ...
          {(owner.vendors ?? []).map(v => (
            <div key={v.id} className="flex items-start gap-3 px-5 py-4">
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                ...
                <button onClick={() => deleteVendor(v.id)} className="p-1.5 text-[#3a5070] hover:text-[#e05c5c] hover:bg-[#2a0e0e] rounded-lg transition-colors"><Trash2 size={13} /></button>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-[#1a2335] rounded-xl border border-[#1e2d45]">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#1e2d45]">
          ...
          <button onClick={onAddOutreach} className="flex items-center gap-1.5 text-xs text-[#4a90d9] hover:text-[#4a90d9] border border-[#1e3a5a] hover:border-[#4a90d9] px-3 py-1.5 rounded-lg transition-colors font-medium"><Plus size={13} /> Log Outreach</button>
        </div>
        <div className="divide-y divide-[#1e2d45]">
        ...
        <div className="bg-[#1a2335] rounded-2xl shadow-2xl w-full max-w-md mx-4 flex flex-col max-h-[80vh]">
          ...
          <div className="flex-1 overflow-y-auto p-5">
            {importLoading ? (
              ...
              <p className="text-sm text-[#e05c5c] text-center py-8">{importError}</p>
          ...
          {!importLoading && !importError && importProps.length > 0 && (
            <div className="px-5 py-4 border-t border-[#1e2d45] space-y-2">
              {importError && <p className="text-xs text-[#e05c5c]">{importError}</p>}
              <button
