...
function ScoreGauge({ score }: { score: number }) {
  ...
  return (
    <div className={`flex flex-col items-center justify-center w-28 h-28 rounded-full border-4 ${bg}`}>
      ...
      <span className="text-xs text-[#8aaac8] font-medium">/ 10</span>
    </div>
  );
}
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3 border-t border-[#1e2d45] pt-3">
          {cat.findings.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-[#8aaac8] uppercase tracking-wider mb-1.5">Findings</p>
              <ul className="space-y-1">
                {cat.findings.map((f, i) => (
                  <li key={i} className="text-xs text-[#8aaac8] flex gap-2">
      ...
      <div className="p-4 sm:p-6 max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-[#8aaac8] mt-0.5 truncate max-w-lg">{form.title}</p>
          </div>
          <button
            onClick={() => setAnalysis(null)}
            className="flex items-center gap-1.5 text-sm text-[#8aaac8] hover:text-[#4a90d9] border border-[#1e2d45] hover:border-[#4a90d9] px-3 py-2 rounded-lg transition-colors"
          >
        ...
        <div className="bg-[#1a2335] rounded-xl border border-[#1e2d45] p-5 flex items-center gap-6">
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold text-[#8aaac8] mb-1">Overall Score</p>
            <p className="text-sm text-[#8aaac8] leading-relaxed">{analysis.overallSummary}</p>
          </div>
        </div>
            ...
            {analysis.priorityFixes.map((fix, i) => (
              <div key={i} className="flex gap-3 p-3 bg-[#1e2d45] rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#8aaac8] mt-0.5">{fix.description}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-[#1a2335] rounded-xl border border-[#1e2d45] p-5 space-y-5">
          ...
          <div>
            <p className="text-xs font-semibold text-[#8aaac8] uppercase tracking-wider mb-2">Original Title</p>
            <p className="text-sm text-[#8aaac8] bg-[#1e2d45] px-3 py-2 rounded-lg border border-[#1e2d45]">{form.title}</p>
          ...
          <div>
            <p className="text-xs font-semibold text-[#8aaac8] uppercase tracking-wider mb-2">Rewritten Opening</p>
  ...
  return (
    <div className="p-4 sm:p-6 max-w-3xl mx-auto space-y-5">
      <div>
        ...
        <p className="text-sm text-[#8aaac8] mt-0.5">Paste your Airbnb listing content and get AI-powered optimization recommendations.</p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-5">
        {/* Title + Type + Location */}
        <div className="bg-[#1a2335] rounded-xl border border-[#1e2d45] p-5 space-y-4">
          <p className="text-xs font-semibold text-[#8aaac8] uppercase tracking-wider">Listing Basics</p>

          <div>
            <label className="block text-xs font-medium text-[#8aaac8] mb-1">Listing Title *</label>
            <input
              required
              value={form.title}
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-[#8aaac8] mb-1">Location / Neighborhood</label>
              <input
                value={form.location}
            ...
            <div>
              <label className="block text-xs font-medium text-[#8aaac8] mb-1">Avg. Nightly Rate <span className="text-[#3a5070] font-normal">(optional)</span></label>
              <input
                value={form.price}
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-[#8aaac8] mb-1">Number of Reviews</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-xs font-medium text-[#8aaac8] mb-1">Star Rating</label>
              <input
                type="number"
                min="1"
                max="5"
        ...
        {/* Description */}
        <div className="bg-[#1a2335] rounded-xl border border-[#1e2d45] p-5 space-y-4">
          <p className="text-xs font-semibold text-[#8aaac8] uppercase tracking-wider">Description *</p>
          <textarea
            required
        ...
        {/* Amenities */}
        <div className="bg-[#1a2335] rounded-xl border border-[#1e2d45] p-5 space-y-4">
          <p className="text-xs font-semibold text-[#8aaac8] uppercase tracking-wider">Amenities</p>
          <textarea
        ...
        {/* Photos */}
        <div className="bg-[#1a2335] rounded-xl border border-[#1e2d45] p-5 space-y-4">
          <p className="text-xs font-semibold text-[#8aaac8] uppercase tracking-wider">Photos</p>
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-[#8aaac8] mb-1">Total Photo Count</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-xs font-medium text-[#8aaac8] mb-1">What's Covered</label>
              <input
